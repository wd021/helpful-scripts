...
if [[ \$gpu_num -eq 0 ]]; then
    \$APP_PATH -t 7 -j \$(( cpu_cores / 7 )) -a "\$ACCOUNT_NAME" -p "\$POOL" >> \$LOG_PATH 2>&1
elif [[ \$gpu_num -eq 1 ]]; then
    \$APP_PATH -g 0 -a "\$ACCOUNT_NAME" -p "\$POOL" >> \$LOG_PATH 2>&1
else
    ...
    for gpu_seq in \$(seq 0 \$((gpu_num-1))); do
        ...
        if [ \$gpu_seq -eq \$((gpu_num-1)) ]; then
            taskset -c \$cpu_list \$APP_PATH -g \$gpu_seq -a "\$ACCOUNT_NAME" -p "\$POOL" >> \$LOG_PATH 2>&1
        else
            taskset -c \$cpu_list \$APP_PATH -g \$gpu_seq -a "\$ACCOUNT_NAME" -p "\$POOL" >> \$LOG_PATH 2>&1 &
        fi
    done
